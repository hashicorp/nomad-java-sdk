package com.hashicorp.nomad.apimodel;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.hashicorp.nomad.javasdk.ApiObject;
import com.hashicorp.nomad.javasdk.NomadJson;

import java.io.IOException;
import java.util.List;
import java.util.Map;

/**
 * This is a generated JavaBean representing a request or response structure.
 *
 * @see <a href="https://www.nomadproject.io/docs/http/index.html">Nomad HTTP API</a> documentation associated with the endpoint you are using.
 */
public final class SearchResponse extends ApiObject {
    private Map<String, List<String>> matches;
    private Map<String, Boolean> truncations;

    @JsonProperty("Matches")
    public Map<String, List<String>> getMatches() {
        return matches;
    }

    public SearchResponse setMatches(Map<String, List<String>> matches) {
        this.matches = matches;
        return this;
    }

    public SearchResponse addMatches(String key, List<String> value) {
        if (this.matches == null)
            this.matches = new java.util.HashMap<>();
        this.matches.put(key, value);
        return this;
    }

    @JsonProperty("Truncations")
    public Map<String, Boolean> getTruncations() {
        return truncations;
    }

    public SearchResponse setTruncations(Map<String, Boolean> truncations) {
        this.truncations = truncations;
        return this;
    }

    public SearchResponse addTruncations(String key, boolean value) {
        if (this.truncations == null)
            this.truncations = new java.util.HashMap<>();
        this.truncations.put(key, value);
        return this;
    }

    @Override
    public String toString() {
        return NomadJson.serialize(this);
    }

    public static SearchResponse fromJson(String json) throws IOException {
        return NomadJson.deserialize(json, SearchResponse.class);
    }

    public static List<SearchResponse> fromJsonArray(String json) throws IOException {
        return NomadJson.deserializeList(json, SearchResponse.class);
    }
}
